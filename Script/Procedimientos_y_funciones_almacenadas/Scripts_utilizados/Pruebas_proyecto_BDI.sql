...
--DELETE FROM Paciente WHERE dni_paciente = 12345678
...
--Comparacion de eficiencia con una consulta directa
INSERT INTO Paciente (nombre_paciente, apellido_paciente,dni_paciente,telefono_paciente,fecha_nacimiento)
VALUES ('Pedro','Gonzales',46317890,379589776,'2000-01-01');
...
AS
BEGIN
	--Con esta sentencia se evita que al eliminar una medicacion aparezca el mensaje de "(1 row(s) affected)", volviendo 
	--este procedimiento de eliminación más amigable al momento de mostrar los datos.
    SET NOCOUNT ON;
...
--Comparación de eficiencia con una consulta directa
DELETE FROM Medicacion
WHERE nombre_medicacion = 'Diazepam';
...
AS
BEGIN
--Con esta sentencia se evita que al editar correctamente la informacion de un usuario aparezca el mensaje de "(1 row(s) affected)", volviendo 
--este procedimiento de edición más amigable al momento de mostrar los datos.
    SET NOCOUNT ON;
...
--Comparación de eficiencia con una consulta directa
UPDATE Usuario
SET nombre_usuario = 'Marcela',
    apellido_usuario = 'Fernández',
    email_usuario = 'marce.fernandez@example.com'
WHERE dni_usuario = 20111222;
